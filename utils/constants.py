...
def adjust_constants_x_coords(offset=405):
    ...
    for name, value in list(g.items()):
        if name.endswith("_REGION") and isinstance(value, tuple) and len(value) >= 2:  # only touch REGION constants
        ...
        if name.endswith("_MOUSE_POS") and isinstance(value, tuple) and len(value) >= 2:  # only touch REGION constants
            # Adjust only the x-coordinates (0 and 2)
            new_value = (
                value[0] + offset,
                value[1],
        ...
        if name.endswith("_BBOX") and isinstance(value, tuple) and len(value) >= 2:  # only touch REGION constants
            # Adjust only the x-coordinates (0 and 2)
            new_value = (
                value[0] + offset,
                value[1],
